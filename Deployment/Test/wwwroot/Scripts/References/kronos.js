...
;(function ($) {
        ...
        function kronos(element, settings){
            ...
            var defaults = {
                ...
                date: {
                    ...
                    disabled: [],
                    holiday: [],
                    xholiday: [] //mod by: @leo 081119
                },
                onChange: function () {
                    //console.log(date);
                },
                restrcDaysTP: { //mod by: @leo 083019 added tooltip
                    title: null,
                    placement: null
                },
                bookedDateTP: { //mod by: @leo 083019 added tooltip
                    title: null,
                    placement: null
                }
            }
    ...
    Kronos.prototype.setDateClasses = function () {
        ...
        _.element.date.date.each(function () {
            ...
            //mod by: @leo 091819 added tool tip on holiday
            this.bookedDateTP = _.options.date.holiday.indexOf(this.core) !== -1
                || _.options.date.holiday.indexOf(this.mmdd) !== -1
                ? { 'data-toggle': 'tooltip', 'data-placement': _.options.bookedDateTP.placement, 'title': _.options.bookedDateTP.title } : '';




            if (this.Class != ' holiday') {
                if (this.core != _.corevar.today) {
                    this.Class += _.options.date.xholiday.indexOf(this.core) !== -1
                        || _.options.date.xholiday.indexOf(this.mmdd) !== -1
                        ? ' xholiday' : '';

                    //mod by: @leo 081119 added tool tip on xholiday
                    this.restrcDaysTP = _.options.date.xholiday.indexOf(this.core) !== -1
                        || _.options.date.xholiday.indexOf(this.mmdd) !== -1
                        ? { 'data-toggle': 'tooltip', 'data-placement': _.options.restrcDaysTP.placement, 'title': _.options.restrcDaysTP.title } : '';

                    $(this).attr(this.restrcDaysTP) //mod by: @leo 081119 added tool tip on xholiday
                }
            }
            
            $(this).attr(this.bookedDateTP) //mod by: @leo 091819 added tool tip on holiday
      
            $('[data-toggle="tooltip"]').tooltip(); //mod by: @leo 081119 added tool tip on xholiday --needed bootstrap 4 
            ...
            date: function (_) {
                ...
                _.element.date.date.on(_.events.click, function () {
                    if (!$(this).hasClass('disabled')) {
                        if (!$(this).hasClass('holiday')) { //mob by: @leo 081119
                            if (!$(this).hasClass('xholiday')) {//mob by: @leo 081119
                                _.element.kronos.val(_.convertFormat(this.core)).attr({ 'core': this.core });
                                _.options.onChange(this.core, $(this).hasClass('xholiday'), this);
                ...
                _.element.date.prev.on(_.events.click, function () {
                    if (!_.initial.actionFlag) {
                        _.setDateInit(-1);
                        if (_.initial.thisM > _.initial.todayM) {
                            _.setDateInit(-1);
                        }
